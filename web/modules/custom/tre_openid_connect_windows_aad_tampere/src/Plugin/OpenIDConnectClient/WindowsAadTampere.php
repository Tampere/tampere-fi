...
namespace Drupal\tre_openid_connect_windows_aad_tampere\Plugin\OpenIDConnectClient;
 ...
 * @OpenIDConnectClient(
 *   id = "tampere_fi",
 *   label = @Translation("Tampere.fi AD account")
 * )
 */
class WindowsAadTampere extends OpenIDConnectClientBase {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['enable_single_sign_out'] = [
      ...
      '#description' => $this->t('Checking this option will enable Single Sign Out to occur so long as the logout url has been set to (http(s)://yoursite.com/openid-connect/tampere_fi/signout) in your Azure AD registered app settings. If a user logs out of the Drupal app then they will be logged out of their SSO session elsewhere as well. Conversely if a user signs out of their SSO account elsewhere, such as Office 365, they will also be logged out of this app.'),
    ];
    ...
    $form['group_mapping'] = [
      ...
      '#states' => [
        'invisible' => [
          ':input[name="clients[tampere_fi][settings][map_ad_groups_to_roles]"]' => ['checked' => FALSE],
        ],
      ],
    ];
    ...
    $form['group_mapping']['mappings'] = [
      ...
      '#states' => [
        'invisible' => [
          ':input[name="clients[tampere_fi][settings][group_mapping][method]"]' => ['value' => 0],
        ],
      ],
    ];
    ...
    $form['userinfo_endpoint_wa'] = [
      ...
      '#states' => [
        'visible' => [
          ':input[name="clients[tampere_fi][settings][userinfo_graph_api_wa]"]' => ['value' => 0],
        ],
      ],
    ];
    $form['userinfo_graph_api_use_other_mails'] = [
      ...
      '#states' => [
        'visible' => [
          ':input[name="clients[tampere_fi][settings][userinfo_graph_api_wa]"]' => ['value' => 1],
        ],
      ],
    ];
  ...
  public function retrieveTokens($authorization_code) {
    ...
    try {
      ...
      $this->loggerFactory->get('tre_openid_connect_windows_aad_tampere')
